...
func runPrototypeProbe(progress: @escaping (String) -> Void) async -> [String: Any] {
  ...
  do {
    ...
    let directory = documents.appendingPathComponent("WasmerWebKitPrototype")
    try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    let script = try String(contentsOf: Bundle.main.resourceURL!.appendingPathComponent("Web/python-smoke.py"), encoding: .utf8)
