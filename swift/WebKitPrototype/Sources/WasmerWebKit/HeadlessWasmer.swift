...
public final class HeadlessWasmer: NSObject, WKNavigationDelegate {
  ...
  public func start() async throws -> WebKitCapabilities {
    ...
    started = true
    #if SWIFT_PACKAGE
    let assets = Bundle.module.url(forResource: "Web", withExtension: nil)!
    #else
    let assets = Bundle.main.resourceURL!.appendingPathComponent("Web")
    #endif
    guard FileManager.default.fileExists(atPath: assets.appendingPathComponent("sdk/pkg/wasmer_sdk_js_bg.wasm").path) else {
      throw WebKitRuntimeError.failed("Run scripts/prototype.py prepare to bundle the SDK assets")
    }
