...
and the JavaScript SDK build prerequisites described in
[WebKitPrototype](../../WebKitPrototype/README.md).

From the repository root:

```sh
# First time, or after changing Rust / wasm-bindgen code:
python3 swift/WebKitPrototype/scripts/prototype.py prepare --rebuild-wasm

python3 swift/Examples/iOSTerminal/build.py run
```

The script builds libghostty-vt from a pinned upstream revision, builds the app,
...
this example does not include a full Unix distribution or package installer.
...
```sh
python3 swift/Examples/iOSTerminal/build.py run --example node
# Or: --example python
```
...
The shared `HeadlessWasmer` prototype adds `startTerminal`, `writeTerminal`,
...
```sh
python3 swift/Examples/iOSTerminal/build.py test
python3 swift/Examples/iOSTerminal/build.py build --platform device
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer swift test --package-path swift/WebKitPrototype
node --test swift/WebKitPrototype/scripts/native-network.test.mjs
```
...
`.build/`, SDK resources, binaries, and test artifacts are ignored by Git.
