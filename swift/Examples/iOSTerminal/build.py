...
PROTOTYPE = ROOT.parents[1] / "WebKitPrototype"
sys.path.insert(0, str(PROTOTYPE / "scripts"))
from prototype import run, output, prepare, select_simulator, SDKS, ENV
...
def build(platform):
    ...
    prepare()
    app = ARTIFACTS / platform / "WasmerTerminal.app"
    ...
    sources = sorted((PROTOTYPE / "Sources/WasmerWebKit").glob("*.swift"))
    run("xcrun", "--sdk", sdk, "swiftc", "-swift-version", "6", "-parse-as-library",
        ...
        *sources, *sorted(ROOT.glob("*.swift")), obj,
        prefix / "lib/libghostty-vt.a", "-o", app / "WasmerTerminal")
    if (app / "Web").exists():
        shutil.rmtree(app / "Web")
    shutil.copytree(PROTOTYPE / "Sources/WasmerWebKit/Web", app / "Web")
    ...
    info = dict(CFBundleIdentifier=BUNDLE_ID, CFBundleExecutable="WasmerTerminal",
                CFBundleName="Wasmer Terminal", CFBundleDisplayName="Wasmer Terminal",
