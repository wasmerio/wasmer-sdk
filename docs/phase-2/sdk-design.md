...
- a registry specification such as `python/python@=3.13.18`;
...
await sandbox.installPackage("python/python@=3.13.18");
...
```ts
const toolbox = await wasmer.packages.load("namespace/toolbox@1.2.3");
await using sandbox = await wasmer.sandboxes.create({
  packages: [toolbox],
});
...
no required side-effectful `init()` call. Call `await wasmer.ready()` only when
...
`python/python@=3.13.18`. Release tests and production guides should use a verified
...
```ts
await init();
const pkg = await Wasmer.fromRegistry("python/python@=3.13.18");
...
```ts
const pkg = await wasmer.packages.load("namespace/toolbox@1.2.3");
