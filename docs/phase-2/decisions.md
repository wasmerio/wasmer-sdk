...
| DX-044 | Make `Command.run()` checked by default: an unsuccessful completion throws `ProcessExitError` containing its `Output`. `check: false` explicitly opts into status-as-data. Keep spawned-process `wait()` unchecked. | Accepted |
