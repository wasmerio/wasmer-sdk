...
class RuntimeTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        python = await self.client.packages.load("python/python@=3.13.20")
        self.assertEqual(python.id, "python/python@3.13.20")
    ...
    async def test_stdin_and_dynamic_install(self) -> None:
        installed = await self.sandbox.install_package("python/python@=3.13.20")
